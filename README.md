...
Current source version: **v0.3.13**. We are intentionally **not publishing to crates.io yet**; install from GitHub release assets or from the GitHub repository.
...
follow-ups that write the same path, normalized alias path, or parent/child path
must be dependency-ordered; unordered write-set conflicts are rejected with
policy evidence instead of becoming graph nodes. Later `execute`
