...
Current source version: **v0.3.3**. We are intentionally **not publishing to crates.io yet**; install from GitHub release assets or from the GitHub repository.
...
| `omk goal` controller scaffold | Current scaffold: creates durable goal state, planning artifacts, task graph, local verification task evidence, bounded Wire-backed agent task and mutation evidence, controller review/security evidence, not-ready proof, and cancellation failure artifacts. |
...
`artifacts/agent-runs/`. `omk goal review` records controller review and
bounded secret-scan security evidence under `artifacts/reviews/`. Task graph
mutation, post-mutation gate reruns, integration, specialist review loops, and
ready proof generation are still planned. The current `team run`, event log, gates, and
...
local gate evidence, bounded Wire-backed agent task/mutation evidence,
...
| Goal runtime | Durable goal state, plan/run/list/status/show/proof/verify/execute/review/cancel, planning artifacts, task graph with controller-owned, local verification, bounded Wire-backed agent mutation, review, and security evidence, git evidence, local gate evidence, not-ready proof, and cancellation failure artifacts. Task graph mutation, post-mutation gate reruns, specialist reviews, and integration loops are next. | Current Scaffold |
