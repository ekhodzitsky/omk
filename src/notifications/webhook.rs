...
fn format_discord(event: &NotificationEvent) -> (String, Vec<serde_json::Value>) {
    match event {
        NotificationEvent::TeamSpawned { name, task, workers, role } => {
        ...
        NotificationEvent::TeamShutdown { name, duration_secs, status } => {
            let embed = serde_json::json!({
                "title": "Team Shutdown",
...
fn format_slack(event: &NotificationEvent) -> String {
    match event {
        NotificationEvent::TeamSpawned { name, task, workers, role } => {
            format!("🚀 *Team Spawned*\n• Name: {}\n• Role: {}\n• Workers: {}\n• Task: {}",
                name, role, workers, task.chars().take(200).collect::<String>())
        }
        NotificationEvent::TeamShutdown { name, duration_secs, status } => {
            format!("🛑 *Team Shutdown*\n• Name: {}\n• Status: {}\n• Duration: {}s",
