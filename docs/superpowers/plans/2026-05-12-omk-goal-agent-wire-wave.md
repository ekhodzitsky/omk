...
**Architecture:** `omk goal execute` first runs the local verification wall. If required gates pass, it creates a bounded agent run under the goal artifacts directory, starts one Wire worker, dispatches the `goal-agent-execute` scheduler task, records worker artifacts, updates the goal task graph, then refreshes the proof. Proof remains `not_ready` until review/security evidence exists.
...
Document that `omk goal execute` now launches a bounded Wire-backed agent wave when gates pass, but readiness remains blocked until review/security evidence exists.
