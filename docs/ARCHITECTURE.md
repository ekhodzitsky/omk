...
| `runtime/goal/` | Goal controller scaffold, task graph, local gates, policy-validated bounded agent waves, agent-proposed follow-up dispatch, pause/resume lifecycle, replayable event timelines, budget checkpoints, wall-clock budget enforcement, and proof state. |
