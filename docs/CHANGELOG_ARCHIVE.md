...
Historical release notes moved out of `CHANGELOG.md` to keep the active
changelog under the workspace review-size limit.
...
- **Wire Protocol types**: Complete Rust type definitions for Kimi Code CLI wire mode (`src/wire/protocol.rs`, now the `kimi-wire` crate) — JSON-RPC 2.0 messages, all 17 event types, 4 request types, display blocks, error codes, initialization handshake.
- **Wire client**: `WireClient` (`src/wire/client.rs`, now the `kimi-wire` crate) spawns `kimi --wire`, sends requests, reads events/requests/responses, handles approval flow. Supports `initialize`, `prompt`, `cancel`, and message loop processing.
